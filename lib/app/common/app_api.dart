...
class ApiRequster{
  ...
  void request(String endPointUrl,int reqMode,int reqCode,{bool daynamicUrl = false,body,bool useToken=false} ) async{
    ...
    //
    // if(sharedHelper.getIslogin()||useToken){
    //   String token = await _getTokenFromSharedPrefence();
    //   if(endPointUrl.contains("?")){
    //     endPointUrl += "&token=$token&userID=${sharedHelper.getUserFavorite()}";
    //   }else{
    //     endPointUrl += "?token=$token&userID=${sharedHelper.getUserFavorite()}";
    //   }
    // }
    ...
    try {
      ...
      switch(reqMode){
        case 1:
          try {
            var response = await dio.get(daynamicUrl?endPointUrl:mainUrl+endPointUrl,);
      ...
      try {
        ...
        // TODO
        print('ApiRequster.request 2= ${ex.response} ');
